...
public class AFilterRunner {
    ...
    protected void runFilter(DataSet set, ReversibleFilter filter, String ext) throws IOException {
        ...
        filter.reverse(set);
        String fileNameR = DATA_FOLDER + this.fileName + ext + "R.arff";
        String[] classNamesR = getClassNamesFromLabelDescription(set.getLabelDataSet().getDescription());
        writeArffHeader(fileNameR, set.getDescription(), classNamesR);
        DataSetWriter wrR = new DataSetWriter(set, fileNameR, true, classNamesR);
        try {
            wrR.write();
        } catch (IOException e) {
            e.printStackTrace();
